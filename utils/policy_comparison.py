        ...
        if self.locals.get('dones')[0]:
            ...
            if 'episode' in info:
                ...
                if len(self.rewards) % 10 == 0:
                    ...
                    print(f"Average episode length over last 10 episodes: {avg_length:.2f}")
                    print("-" * 50)
        return True
...
def main():
    ...
    for result in results:
        stats = result["stats"]
        print(f"{result['policy']:<15} | {stats.get('avg_reward', 'N/A'):<15.2f} | {stats.get('avg_length', 'N/A'):<20.2f} | {stats.get('total_episodes', 'N/A'):<15}")
